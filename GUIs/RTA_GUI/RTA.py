...
class calc_A(AppFrame):
    ...
    def init_commandframe(self):
        ...
        rowcount+=1
        self.reflectance=LoadSingleFile(parent=self.commandframe,ini=self.ini, read=Read_from.ihtm, bg='lightblue', path='load_file_path', write_ini=self.write_ini, text='Load reflectance file', filetypes=[('IHTM E60 file','*.dtsp' )])
        self.reflectance.add_action(self.action_reflectance)
        self.reflectance.grid(row=rowcount,column=1)
        rowcount+=1
        self.transmittance=LoadSingleFile(parent=self.commandframe,ini=self.ini, read=Read_from.ihtm, bg='lightgreen', path='load_file_path', write_ini=self.write_ini, text='Load transmittance file', filetypes=[('IHTM E60 file','*.dtsp' )])
